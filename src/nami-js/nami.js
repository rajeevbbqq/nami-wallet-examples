...
class NamiWalletApi {
    ...
    async getBalance() {
        try {
            ...
            utxos.forEach((element) => {
                ...
                countedValue = countedValue.checked_add(amt);
            });

            const minAda = min_ada_required(
                countedValue,
                false,
                BigNum.from_str(protocolParameter.minUtxo.toString())
            );
    ...
    async transaction({
        ...
        try {
            ...
            for (let recipient of recipients) {
                ...
                let mintedAssets = this._makeMintedAssets(
                    recipient?.mintedAssets || []
                );
    ...
    async _txBuilder({
        ...
        const rawLimit = 20 + totalAssets;
        const limits = rawLimit;

        const selection = await CoinSelection.randomImprove(Utxos, Outputs, limits);
        console.log(selection);
        const inputs = selection.input;
        const txnBuilderConfig = TransactionBuilderConfigBuilder.new()
            .fee_algo(
                LinearFee.new(
                    BigNum.from_str(ProtocolParameter.linearFee.minFeeA),
                    BigNum.from_str(ProtocolParameter.linearFee.minFeeB)
                )
            )
            .pool_deposit(BigNum.from_str(ProtocolParameter.poolDeposit.toString()))
            .key_deposit(BigNum.from_str(ProtocolParameter.keyDeposit.toString()))
            .coins_per_utxo_word(BigNum.from_str(ProtocolParameter.coinsPerUtxoWord.toString()))
            .max_value_size(BigNum.from_str(VALUE_SIZE.toString()))
            .max_tx_size(BigNum.from_str(MULTIASSET_SIZE.toString()))
            .prefer_pure_change(true)
            .build();

        const txBuilder = TransactionBuilder.new(txnBuilderConfig);
        ...
        for (let i = 0; i < inputs.length; i++) {
            const utxo = inputs[i];
            let addr = utxo.output().address().to_bech32()
            const outPutAddr = Address.from_bech32(addr);
            console.log(utxo.input().transaction_id());
            const txnHash = TransactionHash.from_bytes(utxo.input().transaction_id().to_bytes())
            console.log('txn hash', txnHash.to_bech32())
            const txnInput = TransactionInput.new(txnHash, utxo.input().index())
            console.log('txn input', txnInput)
            txBuilder.add_input(
                outPutAddr,
                txnInput,
                utxo.output().amount()
